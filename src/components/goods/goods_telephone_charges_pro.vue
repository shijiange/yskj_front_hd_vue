  ...
  <div class="page">
    ...
    <div class="recharge">
      ...
      <div class="recharge-box">
        <div class="input-box">
          <input type="text" v-model="mobile" placeholder="请输入手机号" @input="tapInpC" />
          <div v-if="mobileCity" class="mobileC">{{ mobileCity }}</div>
        </div>
        <div class="desc">{{ rechargeItem.desc_text }}</div>
        <div class="recharge-title">充值金额</div>
        <div class="recharge-items">
          <div class="recharge-item" @click="handClickOptions(item)" :class="optionsId == item.id ? 'active' : ''" v-for="(item, i) in displayOptions" :key="i" v-if="item.status == 1">
...
export default {
  ...
  created() {
    ...
    //轮播图
    if (this.goods_info && this.goods_info.plugin && this.goods_info.plugin.phoneBillPro) {
      this.bannerUrl = this.goods_info.plugin.phoneBillPro.banner;
    }
  ...
  methods: {
    tapInpC() {
      if(this.mobile.length == 11){
        $http
        .get("plugin.phone-bill-pro.frontend.controller.phone-query.get-phone-info", { mobile: this.mobile })
        .then(response => {
          if (response.result == 1) {
            this.mobileCity = `${response.data.city} ${response.data.isp}`;
          } else {
          }
        })
        .catch(error => {
    ...
    handClickSpecs(item) {
      ...
      console.log(this.displayOptions,'ccccccc')
      this.optionsId = 0;
    },
