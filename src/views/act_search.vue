  ...
  <div id="search-box-first" transition="fadeInLeft">
    ...
    <div class="activity">
      <div class="activity_detail">
        <div class="activity_title">{{ actInfo.simple_tag }}</div>
        <div class="activity_desc">
          <labels label-style="font-size:12px; line-height:16px; color:#fff; border:1px solid #fff;" :labels="actInfo.tags"></labels>
        </div>
        <div class="show_content" @click="activityShow = true">
          活动详情<i class="iconfont icon-advertise-next"></i>
        </div>
        <br/>
        <div class="activity_count_down">
          距离结束还剩
          <van-count-down :time="actInfo.count_down_time" class="bottom_time" format="DD天HH时mm分ss秒" style="color:#fff; display:inline-block;">
      ...
      <van-pull-refresh v-model="isLoading" @refresh="loadTop" success-text="刷新成功">
        ...
        <d-list @load="getMoreData" :finished="!isLoadMore" :loading="loading" :showStatusText="false">
          <act-goods-list style="padding:0 0.75rem;" :goods="goods" bgColor="none" :goods_template="goods_template" text="搜索结果" class="osll" :loading="!isLoadMore" @cartChange="updateActDesc"></act-goods-list>
        </d-list>
      </van-pull-refresh>
    </div>
    ...
    <van-popup v-if="actInfo" v-model="activityShow" class="mint-popup-4 pcStyle_mint" closeable position="bottom" get-container="#appMain">
      <van-nav-bar title="活动详情规则"></van-nav-bar>
      <div style="clear: both;" class="pcStyle_ydC">
        <van-cell-group style="border-width: 0;" inset>
          <div  style=" text-align: left;">
            <div class="activityArr-box">
              <p class="custom-title">活动时间</p>
              <div class="activity-detail"><div class="detail">{{ actInfo.start_time }}至{{ actInfo.end_time }}</div></div>
            </div>
          </div>
          <div  style=" text-align: left;">
            <div class="activityArr-box">
              <p class="custom-title">活动内容</p>
              <div class="activity-detail"><div class="detail">
                {{ actInfo.tags.join('，') }}</div></div>
              <div class="activity-detail"><div class="detail"><br/>活动最终解释权归益生康健所有!</div></div>
            </div>
          </div>
        </van-cell-group>
      </div>
    ...
    <div class="cart_pay">
      <van-notice-bar
        v-if="actInfoDesc.marketing.goods_list_detail != ''"
        :scrollable="false"
        :text="actInfoDesc.marketing.goods_list_detail"
      />
      <van-row>
        <van-col span="8" center >
          <div class="freight">
            合计：<span style="font-size: 14px; color: #f15353;">¥{{actInfoDesc.price}}</span>
          </div>
        </van-col>
        <van-col span="8" class="paysub" offset="8" @click.stop="goToCart">去购物车</van-col>
      </van-row>
    </div>
  </div>
</template>
...
import actGoodsList from "components/actGoodsList";
  ...
  methods: {
    ...
    gotoSearch() {
      ...
      if (!this.fun.isTextEmpty(this.$route.query.fromPage) && this.$route.query.fromPage == 1) {
        ...
        _url = "plugin.marketing.frontend.goods.search-goods";
      }
    ...
    getAct(){
      $http
        .get("plugin.marketing.frontend.act.getActInfo", {act_id: this.$route.query.act_id})
        .then(res => {
          if(res.result == 1){
    ...
    updateActDesc(param){
      console.log(param);
      $http.get('plugin.marketing.frontend.goods.get-act-info', param, true).then(response => {
        if (response.result == 1) {
          if(response.data){
            this.actInfoDesc = response.data;
          }else{
            this.actInfoDesc = {
              price:0,
              marketing:{
    ...
    goToCart() {
      let cart_ids = [];
      for(let i=0; i<this.actInfoDesc.carts.length; i++){
        cart_ids.push(this.actInfoDesc.carts[i].cart_id);
      }
...
.activity{
  background: url('../assets/images/activity_bg.jpeg') no-repeat;
  position: relative;
  .show_content{
    position:absolute;
    right:0;
