    ...
    <div class="top_line">
        ...
        <div class="all_num">
          <div class="today">
            ...
            <div class="today_income">
              <span class="line_num">￥{{ today_income || 0 }}</span
              ><span class="line">今日累计收入</span>
            </div>
          </div>
          <div class="write_off">
            <div class="write_off_num">
              <span class="line_num">{{ total_count || 0 }}</span
              ><span class="line">核销总次数</span>
            </div>
            <div class="all_income">
              <span class="line_num">￥{{ total_income || 0 }}</span
              ><span class="line">累计总收入</span>
            </div>
          </div>
        </div>
      </div>
      <div class="detail_val">
        ...
        <van-cell-group>
          ...
          <van-field v-model="verify_kwd" placeholder="核销员昵称/姓名" />
          <div class="search"><span class="name" @click="btnSearch">搜索</span></div>
        </van-cell-group>
      </div>
      <div class="date">
        <div class="date_detail">
          <van-cell title="选择日期区间" :value="date" @click="showPopup"></van-cell><span style="width: 100%;">{{ start_date_show || '起始时间'}} - {{ end_date_show || '起始时间'}}</span
          >
...
<script>
import store_records_controller from "./store_records_controller";

export default store_records_controller;
</script>
  ...
  .top_line {
    ...
    .van-cell-group {
      ...
      .van-cell {
        ...
        margin: 0.61rem 0.71rem 0.61rem 0;
        padding: 8px 16px;
        flex: 5;
        height: 2.14rem;
        display: flex;
