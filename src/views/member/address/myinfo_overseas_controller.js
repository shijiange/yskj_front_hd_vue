      ...
      $http
        ...
        .then(response => {
          ...
          if (response.result == 1) {
            ...
            this.form.member_card = response.data.idcard.length ? response.data.idcard.replace(/(\d{3})\d*([0-9a-zA-Z]{3})/, '$1******$2') : '' ;
            console.log(response.data.idcard.replace(/(\d{3})\d*([0-9a-zA-Z]{3})/, '$1******$2'))
          } else {
            this.show1 = false;
          }
        })
