...
export default {
  ...
  methods: {
    getJson() {
      ...
      var json = { order_pay_id: this.$route.params.id };
      $http.get('order.merge-pay.dcm-scan-pay', json).then(
        (response)=> {
          // console.log(response)
          if (response.result == 1) {
            this.bank_info = response.data.payee_info;
            this.amount_info = response.data.amount;
            this.pay_info = response.data.pay_sn;
          }else {
            Toast(response.msg);
          }
        },
    ...
    confirmEvent() {
      if (this.upImgSrc != '') {
        ...
        $http.get('plugin.dcm-scan-pay.frontend.pay', json).then(
          function (response) {
            // console.log(response)
            if (response.result == 1) {
              that.$router.push(
