      ...
      >
        <template #reference>
          <span class="my-publish_sub-type">
            ...
            <i
              class="iconfont"
              :class="[showSubTypePopover ? 'icon-member-top' : 'icon-member-bottom']"
            ></i>
          </span>
        </template>
      </van-popover>
...
export default {
  ...
  mounted() {
    let nameSet = this.nameSet;
    for (const key in nameSet) {
      if (this.liveTypesObj[key]) {
        this.liveTypesObj[key]['text'] = nameSet[key] + "记录";
      }
    }
    this.liveTypes = Object.values(this.liveTypesObj);
    this.selectedContentType = this.liveTypes[0];
    this.getData();
  },
  methods: {
    switchTabs(e) {
    ...
    getData() {
      let _url = "plugin.new-media-advertising.frontend.member.get-my-behavior-room-list";
      let _json = {
        page: this.pagination.pages,
        behavior_type: this.selectedContentType.value
      };
