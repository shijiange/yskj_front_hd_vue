<!-- 带货直播 首页装修页组件 -->
<template>
  <div id="livestreaming" :class="className">
    <van-list @load="getData" :finished="pagination.finished" v-model="pagination.loading" :immediate-check="false" >
      <van-tabs v-model="active" swipeable @change="switchTabs">
                ...
                >
                  ...
                  <div class="top">
                    ...
                    <template v-if="item.status == '2'">
                      <van-count-down
                        :time="fun.getTimeDifference(item.live_time)"
                        format="DD天HH时mm分ss秒"
                        v-if="fun.getTimeDifference(item.live_time) != 0"
                      ></van-count-down>
            ...
            <div class="living-box">
              ...
              >
                <li
                  ...
                  v-for="(item, index) in newArray1"
                  :key="index"
                  @click="gotoAnchor(1, null, item.id)"
                >
                  <div class="bg">
                    <img :src="item.cover" alt />
                  </div>
            ...
            <div class="living-box">
              ...
              >
                <li
                  ...
                  v-for="(item, index) in newArray1"
                  :key="index"
                  @click="gotoAnchor(2, null, item.id)"
                >
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let getLiveLoading=this.$toast.loading({
        duration:0
      });
      $http
        .post(_url, _json)
        .then(response => {
          getLiveLoading.clear();
          this.pagination.loading = false;
          if (response.result === 1) {
            let oldLength=this.recordsList.length;
            if (this.active == 1) {
              this.recordsList.push(...response.data);
            } else {
              this.recordsList.push(...response.data.data);
            }
            ...
            // }
            this.sort(oldLength, this.recordsList);
          } else {
            Toast(response.msg);
          }
        })
    ...
    gotoAnchor(_status, _Mid, _Rid, _backId) {
      ...
      if (_status == 1 && _Rid) {
        ...
        this.$router.push(this.fun.getUrl('livePage', { id: _Rid, type: 'livePlay' },{from:"back"}));
      } else if (_status == 3 && _Rid && _backId) {
        //直播回放中，跳转到主播间
        console.log('直播回放中，跳转到主播间回放');
        this.$router.push(this.fun.getUrl('livePage', { id: _Rid, backid: _backId, type: 'playBack' },{from:"back"}));
