<template>
  <div>
    ...
    <van-tabs color="#377aff" @click="getData" v-model="active">
      ...
      <van-tab title="签署中" name="1"></van-tab>
...
export default {
  ...
  activated() {
    if(this.$route.query && (this.$route.query.status || this.$route.query.status==0)){
  ...
  methods: {
      ...
      if(item.is_self === 1) {
        if (item.status < 1) {
          ...
          this.$router.push(
            this.fun.getUrl(
              "launchSign",
              { template_id: item.template_id, id: item.id },
            )
          );
        } else {
    ...
    getData() {
      ...
      //status 3待我签署  1我已签署  2已完成  4已超时 0全部  #70936
      $http
        .get(
          "plugin.yun-sign.frontend.h5.contract.getCompanyList",
          { page: 1,status: this.active, cid: this.$route.query.cid},
          "加载中"
        )
        .then(response => {
          if (response.result === 1) {
