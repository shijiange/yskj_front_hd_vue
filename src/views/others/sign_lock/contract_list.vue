...
export default {
  ...
  methods: {
    toSign(item) {
      this.$router.push(this.fun.getUrl("signPageLock",{id: item.id}));
    },
    stopUse(item) {
      this.$dialog.confirm({ message: "确定取消吗"})
        .then(() => {
          $http
            .get("plugin.yun-contract-lock.frontend.contract.cancel", { id: item.id }, "正在取消")
            .then(response => {
    ...
    downpdf(item) {
      // $http
      //   .get(
      //     "plugin.yun-contract-lock.frontend.h5.contract.download-contract",
      //     { id: item.id },
      //     "加载中"
      //   )
      //   .then(response => {
    ...
    getData() {
      ...
      $http
        .get(
          "plugin.yun-contract-lock.frontend.contract.getWebList",
        ...
        .then(response => {
          if (response.result === 1) {
            ...
            this.total_page = this.dataInfo.last_page || 0;
            this.recordsList = this.dataInfo.data || [];
          } else {
            Toast(response.msg);
          }
        })
