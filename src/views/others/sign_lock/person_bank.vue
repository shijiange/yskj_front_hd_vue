<template>
  <div class="form">
    ...
    <div class="content" v-if="step === 2 || step === 3">
      ...
      <div class="btn" @click="toStep">立即认证</div>
    </div>
...
export default {
  ...
  methods: {
    ...
    newLockUrl() {
      $http.get("plugin.yun-contract-lock.frontend.person.get-auth-page", {
        name: this.form.name,
        id_no: this.form.id_no,
        tel: this.form.tel,
        callback_page: this.fun.getSiteRoot() + "/addons/yun_shop/?menu#/signIndex?i="+this.fun.getKeyByI(),
    ...
    getMemberData() {
      $http
        .post("plugin.yun-contract-lock.frontend.person.get-person-auth-info", {}, "...").then(response => {
          this.step = 2;
          if (response.result === 1) {
    ...
    checkPersonData() {
      $http
        .get("plugin.yun-contract-lock.frontend.person.check-person-approve-by-idNo", {
          name: this.form.name,
          ...
          if (response.result === 1) {
            ...
            if(response.data.status_code === 99) {
              this.$dialog.alert({ message: '您在认证中心已经完成认证，无需重复认证'})
                .then(() => {
                  this.$router.push(this.fun.getUrl("identifyResultLock",{show: 'person'}));
                }).catch(() => {});
            }else {
              this.step = 3;
            }
    ...
    getAgreement() {
      $http
        .post("plugin.yun-contract-lock.frontend.person.get-agreement").then(response => {
          if (response.result === 1) {
    ...
    sureIdentify() {
      ...
      if (this.activeName == 1) {
        url = "plugin.yun-contract-lock.frontend.person.auth-tel-v2";
        json = this.form;
      } else if (this.activeName == 2) {
        url = "plugin.yun-contract-lock.frontend.person.auth-bank-card-v2";
    ...
    sendCode() {
      ...
      let json = {};
      if (this.activeName == 1) {
        url = "plugin.yun-contract-lock.frontend.person.check-tel-sms-code";
        json = {
          flow_id: this.flow_id,
          authcode: this.authcode
        };
      } else if (this.activeName == 2) {
        url = "plugin.yun-contract-lock.frontend.person.check-bank-sms-code";
        this.form.bank_no = this.bank_no;
        json = {
          flow_id: this.flow_id,
          authcode: this.authcode
        ...
        url = "plugin.yun-contract-lock.frontend.person.auth-face-v2";
        json = {
          name: this.form.name,
          id_no: this.form.id_no,
          tel: this.form.tel,
