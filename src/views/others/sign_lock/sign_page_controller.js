...
export default {
  ...
  methods: {
    ...
    isBindMobile() {
      $http
        .get("plugin.yun-contract-lock.frontend.member.get-base-info", {}, "loading")
        .then(response => {
    ...
    getGlobalData() {
      ...
      $http
        .get("plugin.yun-contract-lock.frontend.index.get-public-setting", {}, "loading")
        .then(response => {
          if (response.result === 1) {
    ...
    getData() {
      this.showPage = false;
      $http.post("plugin.yun-contract-lock.frontend.contract.detail-step4", { contract_id: this.contract_id }, "loading").then(response => {
        if (response.result === 1) {
          this.showPage = true;
          this.is_send = response.data.is_send;
          this.user_role_id = response.data.user_role_id;
          this.sign_status = response.data.sign_status;
          this.is_person_auth = response.data.is_person_auth;
          this.is_company_auth = response.data.is_company_auth;
    ...
    onClickLeft() {
      if(this.$route.query.from == 'mobile') {
        this.$router.push(this.fun.getUrl("signIndexLock"));
      }else {
        this.$router.go(-1);
      }
    },
    ...
    toMission() {
      this.$router.push(this.fun.getUrl("signMissionLock", { id: this.contract_id }));
    },
    toIdentify() {
      if(this.is_person_auth == 1) {
    ...
    tapIN() {
      if(this.is_person_auth != 1 || this.is_company_auth != 1) {
        this.toShowContent();
        return;
      }
      if(this.is_send == 1) {
    ...
    newSign() {
      $http.get("plugin.yun-contract-lock.frontend.contract.send", {contract_id: this.contract_id}, "loading").then(response => {
        if (response.result === 1) {
          this.newGetSignUrl();
        } else {
          Toast(response.msg);
    ...
    newGetSignUrl() {
      $http.get("plugin.yun-contract-lock.frontend.contract.sign-page", {contract_id: this.contract_id, success_url: document.location.href}, "loading").then(response => {
        if (response.result === 1) {
    ...
    sendFace() {
      ...
      $http.post("plugin.yun-contract-lock.frontend.contract.get-sign-auth-info", json, "loading").then(response => {
        if (response.result === 1) {
    ...
    sendCode() {
      ...
      $http.post("plugin.yun-contract-lock.frontend.contract.send-code", { contract_id: this.contract_id }, "loading").then(response => {
        if (response.result === 1) {
          const TIME_COUNT = 60;
          if (!this.timer) {
    ...
    companySign() {
      ...
      this.signWait = true;
      this.signFail = false;
      $http.post("plugin.yun-contract-lock.frontend.contract.sign-contract", json, "loading").then(response => {
        this.code = "";
        if (response.result === 1) {
          this.signWait = false;
